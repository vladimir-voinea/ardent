...
class event_cycle_detector {
public:
    event_cycle_detector(TInputType&& input, TCallback&& onCompleteCallback, estd::milliseconds debounceTime = estd::milliseconds{50})
        : m_debounce(std::forward<TInputType>(input), [this](auto newState) { this->onDebouncedStateChange(newState); }, debounceTime),
